...
class HeroRepositoryImpl:HeroRepository {
    ...
    override suspend fun getAllHeroes(page: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = "ok",
            prevPage = calculatePage(page = page)[PREVIOUS_PAGE_KEY],// because the calculate page function returns a map we have to specify the key
    ...
    private fun findHeroes(query: String?): List<Hero> { // where query is the name typed
        val founded = mutableListOf<Hero>()
        return if(!query.isNullOrEmpty()){
            heroes.forEach{ (_, heroes) -> // for each pair or list in our map
              heroes.forEach { hero -> // for each hero
                  if (hero.name.lowercase().contains(query.lowercase())){ // check if
                      founded.add(hero)
                  }
              }
            }
